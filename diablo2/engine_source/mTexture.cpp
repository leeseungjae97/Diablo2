...
namespace m::graphics
{
	...
	HRESULT Texture::GetTexturePathXY(std::wstring path, int x, int y)
	{
	...
	HRESULT Texture::GetTextureInTexture(std::shared_ptr<Texture> texture, Vector2 splitPosition, UINT splitWidth, UINT splitHeight)
	{
		ScratchImage atlasImage;
		HRESULT hr = S_OK;
		atlasImage.Initialize2D(DXGI_FORMAT_R8G8B8A8_UNORM, splitWidth, splitHeight, 1, 1);

		ScratchImage& image = texture->GetScratchImage();
		int spx = static_cast<int>(splitPosition.x);
		int spy = static_cast<int>(splitPosition.y);
		...
		//	, nullptr
		//);
		UINT _splitWidthTop = spx < (splitWidth / 2) ? 0 : spx - (splitWidth / 2);
		UINT _splitHeightTop = spy < (splitHeight / 2) ? 0 : spy - (splitHeight / 2);

		hr = CopyRectangle(*image.GetImage(0, 0, 0), Rect(_splitWidthTop, _splitHeightTop, splitWidth, splitHeight)
	...
	HRESULT Texture::MergeTex2(std::vector<std::shared_ptr<Texture>> mergeTextures, std::vector<Vector2> texturePosition, UINT width, UINT height)
	{
		ScratchImage atlasImage;
		HRESULT hr = S_OK;
		...
		for (std::shared_ptr<Texture> tex : mergeTextures)
		{
			...
			//std::wstring num = std::to_wstring(index);
			//std::wstring fileName = L"t111" + num + L".png";
			//GUID format = GetWICCodec(WIC_CODEC_PNG);
			//hr = SaveToWICFile(
			//	*image.GetImage(0, 0, 0)
			//	, WIC_FLAGS::WIC_FLAGS_NONE
			...
			hr = CopyRectangle(*image.GetImage(0, 0, 0), Rect(0, 0, image.GetMetadata().width, image.GetMetadata().height),
				*atlasImage.GetImage(0, 0, 0), TEX_FILTER_DEFAULT, pos.x, pos.y);
			if (FAILED(hr))
			{
				int a = 0;
			}
